...
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QStyle, QLabel, QTableWidget,
    QHeaderView, QAbstractItemView, QSpacerItem, QSizePolicy, QStackedWidget, QCheckBox, QLineEdit,
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.process_list_button.setIcon(self.style().standardIcon(getattr(QStyle, "SP_ComputerIcon", QStyle.SP_DriveNetIcon)))
        ...
        self.move_to_bottom_button.setIcon(self.style().standardIcon(QStyle.SP_ArrowDown))
        self.move_to_bottom_button.setToolTip("Add selected address(es) to the bottom list")
        ...
        scan_buttons_container_widget.setContentsMargins(0, 0, 0, 0)
        ...
        self.value1_label_double = QLabel("Value:")
        self.value_input1_double = QLineEdit("0") 
        self.and_label_double = QLabel("and")
        self.value2_label_double = QLabel("Value:")
        self.value_input2_double = QLineEdit()
        double_grid_layout.addWidget(self.value1_label_double, 0, 1, Qt.AlignBottom) 
        double_grid_layout.addWidget(self.value2_label_double, 0, 3, Qt.AlignBottom) 
        ...
        double_grid_layout.setColumnMinimumWidth(0, self.hex_label_double.fontMetrics().width("Hex") + self.hex_checkbox_double.sizeHint().width() + 10)
        ...
        self.hex_checkbox_single.stateChanged.connect(lambda state, cb=self.hex_checkbox_double: cb.setChecked(state) if cb.isChecked() != state else None)
        self.hex_checkbox_double.stateChanged.connect(lambda state, cb=self.hex_checkbox_single: cb.setChecked(state) if cb.isChecked() != state else None)
        
        # Scan Type and Value Type
        ...
        self.value_types_map = {
            "Byte": ("b", 1), "2 Bytes": ("h", 2), "4 Bytes": ("i", 4), "8 Bytes": ("q", 8),
            "Float": ("f", 4), "Double": ("d", 8),
            "String": (None, None), "Array Of Byte": (None, None)
        }
        self.value_type_combo.addItems(self.value_types_map.keys())
        
        # String Specific Checkbox
        ...
        self.manual_address_table.setHorizontalHeaderLabels(["Address", "Type", "Value"])
